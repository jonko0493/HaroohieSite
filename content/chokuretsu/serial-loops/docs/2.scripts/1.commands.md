...
Sets or clears a flag. There are tons of possible flags in the game, but the user should stick to using F0-F100 for scratch flags (within a script)
and G1-G20 for global flags (cross-script). Flags beyond these values begin to affect other things in the game (possessed topics, heard BGMs, seen CGs,
etc.).


## `MODIFY_FL`
**Parameters**:
* `Character`: The character whose friendship value is modified (Haruhi, Mikuru, Nagato, Koizumi, or Tsuruya)
* `Value`: The value to add to the friendship level
